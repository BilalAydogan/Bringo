...
export default function EventDetail() {
  ...
  const [assignmentDrafts, setAssignmentDrafts] = useState<
    Record<string, { user_id: string; quantity: number }[]>
  >({});
  ...
  const itemStatusMeta: Record<Item['status'], { label: string; className: string }> = {
    pending: {
      label: t('eventDetail.pending'),
      className: 'bg-neutral-800 text-neutral-300 border-neutral-700',
    },
    assigned: {
      label: t('eventDetail.assigned'),
      className: 'bg-blue-500/10 text-blue-400 border-blue-500/20',
    },
    completed: {
      label: t('eventDetail.completed'),
  ...
  useEffect(() => {
    ...
    Promise.all([axiosInstance.get(`/events/${id}`), axiosInstance.get(`/events/${id}/items`)])
      .then(([eventRes, itemsRes]) => {
        setEvent(eventRes.data.data);
        setItems(itemsRes.data.data ?? []);
      })
      .catch((error: unknown) => setError(getApiErrorMessage(error, t('eventDetail.loadError'))))
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(error, t('eventDetail.deleteError')));
      setDeleting(false);
    }
  };
  ...
  const handleGetInvite = async () => {
    ...
    try {
      ...
      alert(getApiErrorMessage(error, t('eventDetail.inviteError')));
    }
  };

  const handleLeaveEvent = async () => {
    if (!id || !window.confirm(t('eventDetail.leaveConfirm'))) return;
    ...
    try {
      ...
      alert(getApiErrorMessage(error, t('eventDetail.leaveError')));
      setLeaving(false);
    }
  };
  ...
  const handleDeleteItem = async (itemId: string) => {
    if (!window.confirm(t('eventDetail.deleteItemConfirm'))) return;
  ...
  const handleUpdateItem = async (itemId: string) => {
    ...
    try {
      ...
      alert(getApiErrorMessage(error, t('eventDetail.updateItemError')));
    } finally {
      setProcessingItemId(null);
    }
  };
  ...
  const updateDraft = (
    itemId: string,
    index: number,
    patch: { user_id?: string; quantity?: number },
  ) => {
  ...
  const handleSaveAssignments = async (itemId: string) => {
    ...
    if (item?.target_quantity) {
      ...
      if (total > item.target_quantity) {
        alert(t('eventDetail.assignmentOverflow', { total, target: item.target_quantity }));
        return;
      }
    }
    ...
    return (
      <AppLayout>
        <div className="max-w-2xl mx-auto text-center py-24">
          <p className="text-red-400 mb-6">{error || t('eventDetail.notFound')}</p>
  ...
  const assignableUsers = [
    ...
    (user, index, users) =>
      user.id && users.findIndex((candidate) => candidate.id === user.id) === index,
  );
    ...
    <AppLayout>
      <div className="mx-auto max-w-4xl">
        ...
        >
          ...
          {t('eventDetail.events')}
        </Link>

        <div className="rounded-3xl border border-neutral-800 bg-neutral-900 p-4 sm:p-6 lg:p-8">
          <h2 className="mb-6 text-2xl font-bold sm:text-3xl">{event.title}</h2>
          ...
          {event.is_owner ? (
            <div className="mb-8 grid gap-3 sm:grid-cols-3">
              ...
              >
                ...
                {copied ? t('common.copied') : t('eventDetail.inviteCode')}
              </button>
              <button
              ...
              >
                ...
                {deleting ? t('eventDetail.deleting') : t('common.delete')}
              </button>
            </div>
            ...
            <div className="mb-8 flex flex-col gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm text-emerald-300 sm:flex-row sm:items-center sm:justify-between">
              <span>{t('eventDetail.participant')}</span>
              <button
                type="button"
              ...
              >
                ...
                {leaving ? t('eventDetail.leaving') : t('eventDetail.leave')}
              </button>
            </div>
          )}
          ...
          <div className="border-t border-neutral-800 pt-6">
            <div className="mb-6 flex flex-wrap gap-2">
              ...
              >
                {t('eventDetail.participantsLabel')}
              </button>
              <button
                ...
                {event.participants && event.participants.length > 0 ? (
                  <div className="space-y-2">
                    {event.participants.map((participant) => (
                      <div
                        key={participant.id}
                        className="flex flex-col gap-3 rounded-xl border border-neutral-800 bg-neutral-900/70 p-3 sm:flex-row sm:items-center sm:justify-between"
                      >
                        ...
                        >
                          {participant.status === 'accepted'
                            ? t('eventDetail.accepted')
                            : participant.status === 'rejected'
                              ? t('eventDetail.rejected')
                              : t('eventDetail.invited')}
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="rounded-2xl border border-dashed border-neutral-800 bg-neutral-900/60 p-8 text-center">
                    ...
                    <p className="text-neutral-400">{t('eventDetail.noParticipants')}</p>
                  </div>
                )}
              </div>
            )}
                    ...
                    {items.map((item) => {
                      ...
                      const assignedToCurrentUser =
                        item.assignments?.some(
                          (assignment) =>
                            (user?.id && assignment.user?.id === user.id) ||
                            assignment.user?.email === user?.email,
                        ) ?? false;
                              ...
                              {isEditingDetails ? (
                                ...
                                <>
                                  <div className="flex flex-wrap items-center gap-2">
                                    <p
                                      className={`font-medium ${item.status === 'completed' ? 'text-neutral-300 line-through' : 'text-white'}`}
                                    >
                                      {item.name}
                                    </p>
                                    <span
                                      className={`px-2.5 py-1 rounded-lg border text-xs font-medium ${statusMeta.className}`}
                                    >
                                      {statusMeta.label}
                                    </span>
                                    {assignedToCurrentUser && (
                                      <span className="px-2.5 py-1 rounded-lg border border-amber-400/30 bg-amber-400/10 text-xs font-medium text-amber-300">
                                        {t('eventDetail.assignedToYou')}
                                      </span>
                                    )}
                                  </div>
                              ...
                              {isEditingDetails && (
                                <>
                                  ...
                                  <button
                                    ...
                                    disabled={
                                      isProcessing ||
                                      !itemDetailDraft.name.trim() ||
                                      !itemDetailDraft.target_quantity
                                    }
